            ...
            <form @submit.prevent="submit" class="space-y-5">
                <div>
                    <div class="relative">
                        ...
                        <input v-model="form.email" type="email" placeholder="E-mail" class="admin-input pl-12" required />
                    </div>
                </div>
                <div>
                    <div class="relative">
                        <Lock class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                        <input v-model="form.password" type="password" placeholder="Senha" class="admin-input pl-12" required />
                    </div>
                </div>

                <button type="submit" :disabled="form.processing" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2">
...
.admin-input { @apply w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-500 transition-all; }
</style>
